import { AheadBehind, GitInfo, RepositoryOwner } from '@types'
...
export class GitService {
   ...
   * @param repositoryPath
   */
  private async getRepositoryOwner(
    repositoryPath: string,
  ): Promise<RepositoryOwner> {
    try {
      const remoteUrl = await this.getRemoteUrl(repositoryPath)

      if (!remoteUrl) return 'self'

      // Parse GitHub/GitLab URLs
      const match = /github\.com[:/]([^/]+)\/|gitlab\.com[:/]([^/]+)\//.exec(
        remoteUrl,
      )
      if (match) {
