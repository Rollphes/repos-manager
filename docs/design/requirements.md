...
- **プロファイル保存** - 検索・フィルター条件を名前付きプロファイルとして保存
- **プロファイル切り替え** - 保存したプロファイル間の高速切り替えによる動的ビュー生成
- **スマートプロファイル** - 条件に合致するリポジトリが自動的に含まれる動的グループ化
- **共有プロファイル** - チーム間でのフィルタープロファイル設定の共有
- **プロファイル自動適用** - 新しく追加されたリポジトリも条件に合致すれば自動で表示される
- **複合条件プロファイル** - 複数のフィルター条件を組み合わせた高度なプロファイル
- **履歴管理** - プロファイル切り替え履歴と素早いアクセス
- **プロファイルのインポート/エクスポート** - JSON形式での設定のバックアップ・復元
